...
실루엣 풍의 거친 들에서 과보가 되어, 영영 닿을 수 없는 해를 좇는다: 햇빛을 거두어 배율을 올리고, 샘물을 떠 질주를 잇고, 한발과 금오를 부수고, 돌방패의 방향을 읽어 등을 치고, 신력이 차면 한 걸음에 화면 하나를 건너뛴다. 뒤에서 다가오는 긴 밤에 삼켜지기 전에 얼마나 멀리 갈 수 있는가.

흔한 무한 러너와 달리, 천지는 『산해경』의 기록을 끝까지 따라 걷는다 — 새벽의 출발, 황하와 위수를 마시고, 길에서 목말라 죽고, 지팡이를 던져 등림이 된다. 마지막 장을 넘어서도 끝이 아니다. 등림의 달밤과 대황의 별벌판은 충분히 멀리 간 사람만이 본다. **이 이야기는 판을 넘어 쌓인다** — 열두 조각은 한 판에 담기지 않으니, 죽어도 처음으로 돌아가지 않고 다음 판이 지난번 끊긴 곳에서 이어진다.

## 지원 범위

| 항목 | 지원 범위 |
| --- | --- |
| **기기** | 데스크톱(키보드) · 휴대폰 / 태블릿(화면 터치 버튼). 처음 터치하면 전체 화면으로 전환되고 가로로 고정된다. 세로로 들면 먼저 돌리라고 안내한다(iPhone은 예외 — Safari는 웹 페이지의 전체 화면을 허용하지 않는다) |
...
**핵심 흐름**: 앞으로 달린다 → 갈라진 땅과 심연·가시를 넘는다 → 「햇빛」을 거두어 배율을 올리고 신력을 쌓고, 「샘물」을 떠 질주를 잇는다 → 질주로 한발을 부수고 금오를 물리치며, 돌방패는 방향을 읽어 등을 치거나 뛰어올라 밟는다 → 끊이지 않고 처치하면 연속 보너스가 쌓인다 → 신력이 차면 「과보의 도보」로 한 화면을 건넌다 → 뒤쫓는 「긴 밤」에 삼켜지기 전에 더 멀리. 긴 밤에 따라잡히면 한 판이 끝난다.
...
**적**: `한발`(가뭄의 귀신)과 `금오`(열 개의 해의 까마귀). 질주나 도보 중에 부딪히거나 떨어지며 위에서 밟으면 부수어 점수가 되지만, 땅에서 달려 부딪히면 죽는다.

**돌방패 한발**: 돌방패를 진 한발. 정면은 질주를 막아낸다 — 부딪혀도 죽지는 않지만 튕겨 나가 속도를 잃고 질주가 잠시 잠긴다. 긴 밤이 거리를 좁히기에는 충분한 시간이다. 쫓아오지 않고 순찰만 하며 방패는 걸어가는 쪽에 있으니, **어느 쪽으로 걷고 있는지**를 보는 것이 읽기의 전부다. 등 뒤에서 질주하면 「배후 일격」으로 점수가 두 배, 뛰어올라 밟거나 도보로 지나가는 것은 언제나 통한다.

**무리를 이룬다**: 어느 정도 나아가면 적들은 더 이상 홀로 서 있지 않는다 — 서넛이 한 무리로 앞을 막아서니, 하나씩 처리하는 대신 한 무리를 한 번에 풀어야 한다. 돌방패도 무리도 얼마간 달려야 나타난다. 새로운 적이 처음 모습을 드러낼 때는 어떻게 상대하는지 한 줄로 일러 준다.

**연속 처치와 날려보내기**: 4초 안에 이어서 처치하면 연속이 쌓이고 보너스도 함께 오른다(정지 프레임과 화면 흔들림도 무거워진다). 질주나 도보로 쓰러뜨린 적은 뒤로 날아가며, 그 시체가 지나가는 길의 적을 함께 쓸어버린다 — 무리에 한 번 파고들면 여러 마리가 연쇄로 쓰러진다. 다만 돌방패는 날아오는 시체도 똑같이 막아낸다. 정면으로 부딪히면 부서질 뿐이니 등 뒤에서 와야 한다. 밟기는 아래로 으깨는 것이라 시체가 날지 않는다.

**어떻게 죽었는지 본다**: 죽음이 곧바로 화면을 넘기지 않는다. 세계가 멈추고 카메라가 다가가며, 주인공은 그 자리에 쓰러진다. 낭떠러지로 떨어지면 카메라도 따라 내려가 어느 구덩이에 삼켜졌는지 보여 준다. 사인이 먼저 떠오르고, 이윽고 화면이 긴 밤으로 가라앉았다가 결말 그림과 기록이 어둠에서 떠오른다. 기다리기 싫으면 아무 키나 눌러 건너뛴다.
...
**모습 바꾸기**: 제목 화면과 결산 화면 모두에 「모습 바꾸기」가 있다. `옥토끼` `응룡` `형천` `예` `비천` `고양이` 여섯 실루엣이 들어 있고, 「＋」를 누르면 자기 그림을 주인공으로 삼을 수도 있다. **배경이 투명한 PNG나 SVG를 권한다** — 주인공은 땅을 딛고 실루엣 풍경 위에 겹쳐 그려지므로, 불투명한 그림은 네모난 판으로 보인다. SVG는 `viewBox`만 있고 `width`/`height`가 없어도 올바르게 맞춰진다. 그림은 브라우저 안에서 줄여 로컬에 남으며 **올라가지 않는다**. 한 장뿐인 모습에는 달리는 율동을 프로그램이 붙여 준다 — 흔들리는 것은 다리뿐, 머리는 거의 고정된다. 도약에서는 늘어나고 질주에서는 앞으로 기운다. 그러니 얼굴 사진을 넣어도 눈이 어지럽지 않게 제법 그럴듯하게 달린다.
