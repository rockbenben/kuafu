...
| **저장** | 최고 기록과 언어 설정은 브라우저 로컬에 남고, 올라가지 않는다 |
...
**칭호와 공유**: 그 판의 공업에 따라 칭호가 주어진다 — `거친 들에 나서다` → `해를 좇는 자` → `황하와 위수를 마신 자` → `북으로 대택을 마시다` → `과보의 뜻` → `해와 빛을 나란히`. 결산 화면에서 `F`를 누르거나 위쪽 절반을 누르면 기록 카드(결말 그림 + 공업 + 칭호 + 주소)가 만들어지고, 그대로 공유할 수 있다.
